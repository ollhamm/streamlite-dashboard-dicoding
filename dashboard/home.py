...
from fn import AnalisisData, GeoLocations
sns.set(style='white')

# import all_data_done
date_time_col = ["order_approved_at", "order_delivered_carrier_date", "order_delivered_customer_date", "order_estimated_delivery_date", "order_purchase_timestamp", "shipping_limit_date"]
all_data = pd.read_csv('../data/all_data_Done.csv')
...
# Sidebar streamlit
with st.sidebar:
    st.title('Ilham Kusmayadi')

    st.image('../data/ilham.png')

    # Date Range
    start_date, end_date = st.date_input(
        label= 'Select Date in Range',
...
# content seleect datetime
content_df = all_data[(all_data["order_approved_at"] >= str(start_date)) & 
                 (all_data["order_approved_at"] <= str(end_date))]
                 


function = AnalisisData(content_df)
map_plot = GeoLocations(geo_data, plt, mpimg, urllib, st)
...
sns.barplot(x=review_score.index, 
            ...
            palette=["#068DA9" if score == most_common_review_score else "#D3D3D3" for score in review_score.index]
            )

plt.title("Review Scores", fontsize=15)
plt.xlabel("Score")
plt.ylabel("Count")
...
with tab3:
    ...
    with st.expander("See Explanation"):
        st.write('banyak pelanggan di bagian tenggara dan selatan. Informasi lainnya, ada lebih banyak pelanggan di kota-kota yang merupakan ibu kota (São Paulo, Rio de Janeiro, Porto Alegre, dan lainnya).')

st.caption('Copyright (C) Ilham kusmayadi 2024')
